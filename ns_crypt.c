/*
 * Pretty much ripped straight from the implementation found in the OpenSolaris
 * libsldap LDAP client, minus the thread-safe stuff.
 * 
 * CDDL HEADER START
 *
 ...
 * CDDL HEADER END
 */
/* Copyright 2013 Rob A. Shinn.  All rights reserved. 
...
{
    char *cryptedStr;
    if (argc > 1) { /* string to be encoded/decoded in first argument */
        cryptedStr=malloc(strlen(argv[1])+1);
        strncpy(cryptedStr, argv[1], strlen(argv[1])+1);
    } else { /* complain loudly */
        printf("%s: no value provided\n", argv[0]);
        return 1;
    }   
    if (is_cleartext(cryptedStr) == TRUE) { /* encode if not coded */
        printf("%s\n", evalue(cryptedStr));
    } else {
        printf("%s\n", dvalue(cryptedStr)); /* otherwise decode */
    }
    free(cryptedStr);
    return 0;
}
